...
func (ci *ConformanceImage) Pull() error {
	...
	if present, err := ci.Runtime.IsImagePresent(ci.Image); err != nil {
		...
		return errors.New("Failed to detect the pulled image.")
	}

	return nil
}

func (ci *ConformanceImage) List() error {
	images, err := ci.Runtime.ListImage(ci.Image, nil)
	if err != nil {
		return err
	} else {
		fmt.Println(images)
...
func (ci *ConformanceImage) Remove() error {
	...
	if present, err := ci.Runtime.IsImagePresent(ci.Image); err != nil {
		...
		return errors.New("Failed to remove the pulled image.")
	}
